...
export class DatabaseRequest {
  ...
  static collections(database: string): Promise<any> {
    return serverRequest().POST().path(`database/database/${database}/collections`).sendJson();
  }
  ...
  static import(body: any): Promise<any> {
    return serverRequest().POST().path('database/imports').bodyJson(body).sendJson();
  }

  static export(database: string): Promise<any> {
    return serverRequest().path(`database/database/${database}/exportv2`).sendJson();
  }
}
