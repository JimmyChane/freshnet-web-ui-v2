...
import { optString, textContains, trimStringAll, trimText } from '@/utils/U';
...
export class ServiceCustomer {
  ...
  comparePhoneNumber(item: ServiceCustomer): number {
    if (this.phoneNumber && item.phoneNumber) return this.phoneNumber.compare(item.phoneNumber);
