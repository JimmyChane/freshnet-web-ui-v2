...
const isSortDateCreated = computed(() => props.sortMode === SortMode.DateCreated);
const isSortName = computed(() => props.sortMode === SortMode.Name);
const isSortPhoneNumber = computed(() => props.sortMode === SortMode.PhoneNumber);
...
function groupsOfDate(items: Service[]): { title: string; items: Service[] }[] {
  return items.reduce((groups: { title: string; items: Service[] }[], item) => {
...
<template>
  <div class="ListServices">
    <div
      ...
      :key="group.title"
    >
      <FloatingTimestamp class="ListServices-group-title" v-if="group.title" :title="group.title" />

      <div class="ListServices-items">
        <div class="ListServices-items-header" v-if="isDetailView && index === 0">
          <span
