...
import { INFO, INITIAL, PURCHASE, QUOTATION } from '@/data/ServiceEventMethod';
...
import WindowUpdateEventDescription from '@/dialog-components/service/WindowUpdateEventDescription.vue';
  ...
  const menus: Menu[] = [
    {
      ...
      click: isInitial.value ? clickEditServiceDescription : clickEditDescription,
    },
  ];
...
function methodContext(property: string) {
  ...
  if (props.event.isQuotation()) return (QUOTATION as Record<string, any>)[property];
  if (props.event.isPurchase()) return (PURCHASE as Record<string, any>)[property];
  return null;
}
      ...
      <div class="ItemEvent-left">
        <div class="ItemEvent-infos">
          <span v-for="(info, index) in infoTexts" :key="info">
            {{ info }}
            <div class="ItemService-dot" v-if="index < infoTexts.length - 1"></div>
          </span>
        </div>
