...
const props = withDefaults(defineProps<{ inputName?: string; inputPhoneNumber?: string }>(), {
  inputName: '',
  inputPhoneNumber: '',
});
...
async function input() {
  ...
  const templates = customerTemplates.value.reduce((templates: Customer[], template) => {
    ...
    <div class="LayoutFindCustomer-body">
      <div class="LayoutFindCustomer-header">
        ...
        <button class="LayoutFindCustomer-close" @click="() => clickClose()">
          <img class="LayoutFindCustomer-close-img" src="@/assets/icon/close-000000.svg" />
        </button>
      </div>
      <div class="LayoutFindCustomer-list">
        <button
          ...
          :key="`${customerSuggestion.name}${
            customerSuggestion.phoneNumber ? customerSuggestion.phoneNumber.toString() : ''
          }`"
        ...
        >
          ...
          <div
            ...
            v-if="customerSuggestion.name && customerPhoneNumberStr(customerSuggestion)"
          ></div>
          <span>
            {{ customerSuggestion.phoneNumber ? customerSuggestion.phoneNumber.toString() : '' }}
          </span>
