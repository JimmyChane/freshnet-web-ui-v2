...
import { Type, TypeKey } from '@/data/Specification';
...
const props = withDefaults(defineProps<{ width: number; height: number; product: Product }>(), {
  width: 0,
  height: 0,
});
...
const specificationKeys = computed(() => Object.values(TypeKey));
...
const specifications = computed(() => {
  ...
  return props.product.specifications
    ...
    .sort((spec1, spec2) => {
      ...
      let index1 = key1 !== undefined ? specificationKeys.value.indexOf(key1) : -1;
      let index2 = key2 !== undefined ? specificationKeys.value.indexOf(key2) : -1;
...
const gifts = computed(() => {
  ...
  return gifts.map((gift) => optString(gift).trim()).filter((gift) => gift.length);
});
...
function invalidateHeight() {
  ...
  if (height > parentHeight && previousHeight.value !== height) {
    ...
    paddingHorizontal.value = paddingHorizontal.value - initialPadding.value / 10;
...
function obtainKeyOfSpecificationType(type?: TypeKey | Type): TypeKey | undefined {
    ...
    >
      ...
      >
        ...
        <div class="ExportLayoutTwo-header-right">
          <span class="ExportLayoutTwo-price" v-if="price && price.to">{{ price.to }}</span>
