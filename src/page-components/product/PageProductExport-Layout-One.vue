...
function invalidateHeight() {
  ...
  if (height > parentHeight && previousHeight.value !== height) {
    ...
    specificationGap.value = specificationGap.value - initialSpecificationGap.value / 10;
    setTimeout(() => invalidateHeight(), 100);
  }
}

function obtainKeyOfSpecificationType(type?: string | Type): TypeKey | undefined {
  if (typeof type === 'object') return type.key;
  if (typeof type === 'string') return type as TypeKey;
  return undefined;
}
