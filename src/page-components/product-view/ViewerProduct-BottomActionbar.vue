...
async function invalidate() {
  ...
  const contactCall = contact?.links.find((link) => link.socialMedia?.title === 'Call');
  const contactWhatsapp = contact?.links.find((link) => link.socialMedia?.title === 'Whatsapp');
...
<template>
  <div class="ViewerProduct-BottomActionbar" :isButtonTopHidden="parentScrollTop <= 10">
    <ButtonContact
