...
const isPromotion = computed(() => props.product?.isPricePromotion() ?? false);
const isAvailable = computed(() => props.product?.isStockAvailable() ?? false);
const isSecondHand = computed(() => props.product?.isStockSecondHand() ?? false);
...
<template>
  <div class="ProductViewerPriceViewer-parent">
    <Section
      ...
      v-if="product && (title || subtitle || !isAvailable || isPromotion || isSecondHand)"
    >
      ...
      <div class="ProductViewerPriceViewer-tag">
        ...
        <span class="ProductViewerPriceViewer-tag-promotion" v-if="isPromotion">Promotion</span>
        <span class="ProductViewerPriceViewer-tag-secondHand" v-if="isSecondHand">Second Hand</span>
      </div>
    </Section>
  </div>
</template>
