...
function shouldIncludeNextLine(item: Record<string, any>, key: string) {
  return (
    ...
    (isObject(item) && itemKeys.value.indexOf(key) !== itemKeys.value.length - 1)
  );
}
...
<template>
  <div class="ItemObject">
    <span class="ItemObject-property" v-for="key in itemKeys" :key="key">
      ...
      <span class="ItemObject-value" v-else-if="isBoolean(item[key]) || isNumber(item[key])">
        {{ item[key] }}
      </span>
      <span class="ItemObject-value" v-else>{{ `"${item[key]}"` }}</span>

      <span class="ItemObject-nextLine">{{ shouldIncludeNextLine(item, key) ? ',' : '' }}</span>
    </span>
  </div>
</template>
