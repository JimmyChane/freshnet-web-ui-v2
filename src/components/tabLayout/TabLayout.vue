...
import TabVue, { type Tab } from './TabLayout-Tab.vue';

withDefaults(defineProps<{ menus?: Tab[]; isScreenWide?: boolean }>(), {
  menus: () => [],
  isScreenWide: true,
});
</script>
...
<template>
  <div :class="['TabLayout', 'scrollbar']">
    <div>
      <TabVue v-for="menu of menus" :key="menu.title" :menu="menu" :isScreenWide="isScreenWide" />
    </div>
  </div>
</template>
