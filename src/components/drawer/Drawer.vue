...
const isFixedExpand = computed(() => props.mode === Mode.FIXED_EXPAND || props.mode === Mode.FIXED);
const isFixedCollapse = computed(() => props.mode === Mode.FIXED_COLLAPSE);
const isDrawer = computed(() => isDrawerExpand.value || isDrawerCollapse.value);
...
const classModes = computed(() => {
  ...
  if (isDrawerCollapse.value) return ['Drawer-DRAWER', 'Drawer-DRAWER_COLLAPSE'];
  return '';
});
...
.Drawer-DRAWER_COLLAPSE {
  pointer-events: none;
  .Drawer-body {
    transform: translateX(calc(var(--x-start) + var(--drag-x))) translateY(var(--y-start));
  }
  .Drawer-dismissableBox {
    display: none;
  }
