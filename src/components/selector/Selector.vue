...
export interface SelectorMenu {
  key: string;
  title: string;
  icon?: { white?: string; color?: string } | string;
  color?: string;
}
...
const currentColor = computed(() => {
  if (!currentMenu.value || typeof currentMenu.value.primaryColor !== 'string') {
    return 'hsl(0, 0%, 96%)';
  }
...
function invalidate() {
  ...
  menus.value = props.list.map((item) => {
    ...
    const icon = typeof item.icon === 'string' ? item.icon : item.icon?.color;
    return {
      key: item.key,
      title: item.title,
      icon: icon ?? '',
...
<template>
  <Menu
    :class="['transition', 'Selector', isShow ? 'Selector-isSelected' : 'Selector-isDeselected']"
