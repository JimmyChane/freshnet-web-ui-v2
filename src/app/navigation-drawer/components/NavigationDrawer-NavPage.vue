...
const style = computed(() => {
  ...
  if (isWide.value) {
    ...
    style['border-radius'] = hasChildren.value || !isSelected.value ? '0.5rem' : '0';
...
<template>
  <div :class="['NavPage', 'transition']" :style="style">
    ...
    <div class="NavPage-children" v-if="hasChildren" :style="styleChildren">
      <div class="NavPage-sectionParent" v-for="(group2, index) in group2s" :key="group2.key">
        <NavViewGroup :group2="group2" />
        <div class="NavPage-sectionParent-line" v-if="index < group2s.length - 1"></div>
      </div>
    </div>
  </div>
</template>
