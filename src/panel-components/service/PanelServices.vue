...
import { LIST, ServiceState } from '@/data/ServiceState';
...
import ListServices from '@/page-components/service/ListServices.vue';
...
const items = computed(() => stateMenus.value[stateMenuIndex.value]?.list ?? []);
...
const isCurrentStatePending = computed(() => currentState.value === 'pending');
...
function setPageSelected(state: string) {
  ...
  const states = LIST.map((state) => state);
...
function invalidateList() {
  ...
  setTimeout(() => {
    if (PanelGroupServiceTabs.value) PanelGroupServiceTabs.value.scrollLeft = 0;
  }, 2000);
}
...
<template>
  ...
  >
    ...
    <div class="PanelServices-toolbar" v-if="isCurrentStatePending">
      ...
      <button @click="() => emits('clickImport')" v-if="isCurrentUserAdmin">Import Service</button>
    </div>
    ...
    <Empty v-if="!items.length && !useServiceStore().isLoading" :icon="iconEmpty" />
  </div>
</template>
