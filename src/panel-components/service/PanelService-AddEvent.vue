...
import { INFO, PURCHASE, QUOTATION } from '@/data/ServiceEventMethod';
...
const isMethodInfo = computed(() => eventMethod.value === INFO.key);
const isMethodQuotation = computed(() => eventMethod.value === QUOTATION.key);
const isMethodPurchase = computed(() => eventMethod.value === PURCHASE.key);

const methodMenu = computed(() => methodMenus.value.find((menu) => menu.key === eventMethod.value));
...
function invalidateMethod() {
  ...
  if (isMethodQuotation.value || isMethodPurchase.value) InputAmount.value.focus();
}
  ...
  >
    ...
    <div class="AddEvent-footer" :style="{ 'grid-area': 'footer' }">
      ...
      <input
        ...
        v-model="nameOfUser"
      />
      <span v-else class="AddEvent-user" :style="{ 'grid-area': 'user' }">{{ nameOfUser }}</span>
    </div>

    <div class="AddEvent-confirm" :style="{ 'grid-area': 'confirm' }">
      <button class="transition" :style="{ 'grid-area': 'enter' }" @click="() => submit()">
        Enter
      </button>
      <button :style="{ 'grid-area': 'clear' }" @click="() => clear()">Discard</button>
    </div>
