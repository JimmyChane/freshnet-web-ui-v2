...
import ItemProduct from '@/page-components/product/ItemProduct.vue';
import type { TabMenu } from '@/page-components/product/CategoryTab';
import ActionBarProduct from '@/page-components/product/ActionBarProduct.vue';
...
const isEmpty = computed(() => !isLoading.value && !productGroups.value.length);
...
const initRightMenus = computed(() => {
  ...
  if (isEditable.value) {
    ...
    if (menuStock) {
      menus.push({
        title: menuStock.menu?.title === 'All' ? 'Showing All' : 'Showing Available',
        isHidden: true,
        click: () => {
          if (menuStock.menu) {
...
async function invalidate() {
  ...
  if (isEditable.value) {
    filterMenus.value.push(
      new MenuGroup('stock', 'Stock', [{ key: 'all', title: 'All' }, { title: 'Available' }]),
    );
  }
...
<template>
  <div class="PanelProducts" ref="refSelf">
    ...
    <div class="PanelProducts-body">
      <span class="PanelProducts-body-title" v-if="categoryTabs.length > 0">Category</span>
      <TabLayout v-if="categoryTabs.length > 0">
        <CategoryTabVue v-for="menu of categoryTabs" :key="menu.title" :menu="menu" />
      </TabLayout>

      <span class="PanelProducts-body-title" v-if="brandTabs.length > 0">Brand</span>
