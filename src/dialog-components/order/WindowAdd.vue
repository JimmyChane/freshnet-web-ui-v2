...
function createOrder() {
  ...
  if (customer_name.value.trim()) order.customer_name = customer_name.value.trim();
  if (phone_number.value.trim()) order.phone_number = phone_number.value.trim();
...
<template>
  <div class="pageOrder-dialog-newOrder-body">
    ...
    />
    <Input class="PageOrder-input" label="Customer Phone Number" v-model="phone_number" />
    ...
    <div class="buttons">
      <div class="body">
        <button class="button-focus-none" @click="() => popupWindow.close()">Cancel</button>
      </div>

      <div class="body">
        <button class="button-focus transition" @click="() => createOrder()">Submit</button>
      </div>
    </div>
  </div>
</template>
