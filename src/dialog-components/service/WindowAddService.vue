...
const userIsDefault = computed(() => user.value.isDefault() || user.value.isDefault());
...
function clickCustomerSuggestion(customer: Customer) {
  ...
  data.value.customerPhoneNumber = customer.phoneNumber ? customer.phoneNumber.toString() : '';
}
