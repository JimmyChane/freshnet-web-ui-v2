...
const primaryColorBackground = ref(() => props.primaryColor.mix('ffffff', 0.8));
const primaryColorBackgroundHover = ref(() => props.primaryColor.mix('ffffff', 0.6));
const primaryColorBackgroundSelected = ref(() => props.primaryColor.mix('ffffff', 0.4));
...
<template>
  ...
  >
    <button class="LabelMenus-main" @click="() => toggle()" @blur="() => collapse(200)">
      ...
      <img class="LabelMenus-arrow transition" src="@/assets/icon/arrowDown-FFFFFF.svg" />
    </button>
    ...
    >
      <button
        ...
        :style="{
          ...
          '--background-color-selected': primaryColorBackgroundSelected.toString(),
        }"
        :key="menu.title"
        v-for="menu in parsedMenus"
        :class="[
          menu.key === menuKey ? 'LabelMenus-menu-isSelected' : 'LabelMenus-menu-isDeselected',
        ]"
        @click="
          () => {
        ...
        "
      >
        <img class="LabelMenus-menu-icon" v-if="menu.icon" :src="menu.icon ? menu.icon : ''" />
