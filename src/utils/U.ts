...
import type { IconPack } from '@/data/IconPack';
...
export function replace(currentQuery: Record<string, any>, pendingQuery?: Record<string, any>) {
...
export function textContains(text: string = '', keyword: string = ''): boolean {
  return typeof text === 'string' && text.toLowerCase().replace(' ', '').includes(keyword);
}
...
export function parseIcon(icon: Record<string, Icon> | any): IconAsset | null {
  ...
  const light = icon.light instanceof Icon ? icon.light.toUrl() : serverIcon(icon.light).toUrl();
  const dark = icon.dark instanceof Icon ? icon.dark.toUrl() : serverIcon(icon.dark).toUrl();
...
export function parseGroup2s(array: GroupAsset[]): NavViewGroup[] {
  return optArray(array).map((obj: GroupAsset) => {
    ...
    if (obj.userPermissions) navViewGroup.setUserPermissions(obj.userPermissions);
