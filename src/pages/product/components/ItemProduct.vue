<script setup lang="ts">
  ...
  const primaryColor = computed(() => {
    return chroma.valid(primaryColorHex.value)
  ...
  const specLabels = computed(() => {
    return ProductPreset.generateSpecificationLabels(props.item);
  });
  const labels = computed(() => {
    return ProductPreset.generateStockLabels(props.item).map((label) => {
  ...
  async function invalidateFullTitle() {
    ...
    if (props.item) {
      fullTitle.value =
        (await props.item.fetchFullTitle()?.catch(() => null)) ?? "";
    }
  }
