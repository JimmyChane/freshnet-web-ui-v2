<script setup lang="ts">
  ...
  const imagePreview = computed(() => {
    ...
    return images.value.length
      ? images.value[imagePreviewIndex.value]
      : undefined;
  });
  const images = computed(() => props.product?.images ?? []);
  const hasImagePrevious = computed(
    () => images.value.length > 0 && imagePreviewIndex.value > 0,
  );
  const hasImageNext = computed(
    () => images.value.length - 1 > imagePreviewIndex.value,
  );
  ...
  const primaryColor = computed(() =>
    chroma.valid(primaryColorHex.value)
      ? chroma(primaryColorHex.value)
      : chroma("bbbbbb"),
  );
...
<template>
  ...
  >
    <div
      ...
      :style="{ 'background-color': actionbarColor.toString() }"
    >
      <Actionbar
    ...
    <div class="ViewerProduct-header">
      <div class="ViewerProduct-preview" v-if="product">
        <ProductViewerImagePreview
          ...
          @click-image="
            (image) => useImageViewerStore().show({ image, thumbnails: images })
          "
          @click-previous="() => clickPreviousImage()"
          @click-next="() => clickNextImage()"
          @click-remove="
            (image) => emits('click-product-imageRemove', { product, image })
          "
        />
        <ProductViewerImages
