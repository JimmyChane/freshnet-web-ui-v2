...
import {
  ...
  Orientation,
  Size,
} from '@/page-components/product-export/PageProductExport';
...
const orientation = computed(() => options.value[0].selectedItem as Orientation | undefined);
const size = computed(() => options.value[1].selectedItem as Size | undefined);
const row = computed(() => options.value[2].selectedItem as GridCount | undefined);
const column = computed(() => options.value[3].selectedItem as GridCount | undefined);
const layout = computed(() => options.value[4].selectedItem as Layout | undefined);

const isPortrait = computed(() => orientation.value === Orientation.Portrait);

const canvasWidth = computed(() => (isPortrait.value ? size.value?.width : size.value?.height));
const canvasHeight = computed(() => (isPortrait.value ? size.value?.height : size.value?.width));
const canvasRowCount = computed(() => row.value?.count);
const canvasColumnCount = computed(() => column.value?.count);

const itemWidth = computed(() => {
  if (canvasWidth.value === undefined || canvasColumnCount.value === undefined) return 0;
  return canvasWidth.value / canvasColumnCount.value;
});
const itemHeight = computed(() => {
  if (canvasHeight.value === undefined || canvasRowCount.value === undefined) return 0;
...
<template>
  <div class="PageProductExport" v-if="allowEdit">
    ...
    <div class="PageProductExport-body">
      ...
      >
        <LayoutOne v-if="isLayoutOne" :width="itemWidth" :height="itemHeight" :product="product" />
        <LayoutTwo v-if="isLayoutTwo" :width="itemWidth" :height="itemHeight" :product="product" />
      </PrintContent>
