...
import WindowAdd from '@/dialog-components/order/WindowAdd.vue';
import Loading from '@/components/loading/Loading.vue';
import Actionbar from '@/page-components/order/Actionbar.vue';
import SectionOrder from '@/page-components/order/SectionOrder.vue';
...
<template>
  ...
  >
    <Actionbar
      ...
      :title="$options.title"
      :items="items"
      @click-item="(item) => useRouteStore().replaceQuery({ query: { order: item.id } })"
    ...
    <main>
      <SectionOrder
        ...
        @click-collapse="(item) => useRouteStore().replaceQuery({ query: { order: null } })"
        @click-expand="(item) => useRouteStore().replaceQuery({ query: { order: item.id } })"
        @click-complete="(item) => useOrderStore().updateToCompletedOfId(item.id)"
        @click-remove="(item) => useOrderStore().removeOItemOfId({ id: item.id })"
      />
